...
export function createMixedExamVariants({
  exams = [],
  examType = '',
  sourceExamIds = [],
  quotas = {},
  ...
  if (sourceExams.length < 2) {
    return { drafts: [], error: 'Vui lòng chọn ít nhất 2 đề nguồn cùng loại HSA/TSA.' };
  }

  const drafts = [];
  for (let index = 0; index < sourceExams.length; index++) {
    ...
    if (result.error) {
      return {
        drafts: [],
        error: `Không tạo được đề xáo ${index + 1}/${sourceExams.length}: ${result.error}`,
      };
    }

    drafts.push({
      clientId: createDraftId(random),
      label: `Đề xáo ${index + 1}`,
