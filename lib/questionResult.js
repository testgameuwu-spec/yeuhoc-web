...
export function normalizeMAAnswer(answer) {
  if (!answer) return [];
  if (typeof answer === 'object' && !Array.isArray(answer)) return [];
  if (!Array.isArray(answer) && /^none$/i.test(String(answer).trim())) return [];

  const values = Array.isArray(answer)
    ? answer
    : String(answer).toUpperCase().match(/[A-Z]/g) || [];
...
export function hasSubmittedAnswer(question, selectedAnswer) {
  ...
  if (question.type === 'MA') {
    return normalizeMAAnswer(selectedAnswer).length > 0;
  }
...
export function hasCompletedAnswer(question, selectedAnswer) {
  if (!question) return false;

  if (question.type === 'TF') {
    if (!selectedAnswer || typeof selectedAnswer !== 'object') return false;
    const statementKeys = Array.isArray(question.statements) && question.statements.length > 0
      ? question.statements.map((_, index) => getStatementKey(index))
      : Object.keys(question.answer || {});

    if (statementKeys.length === 0) return hasSubmittedAnswer(question, selectedAnswer);
    return statementKeys.every((key) => selectedAnswer[key] === 'D' || selectedAnswer[key] === 'S');
  }

  if (question.type === 'DRAG') {
    if (!selectedAnswer || typeof selectedAnswer !== 'object') return false;
    const blankIds = getDragBlankIds(question.content);
    const expectedBlankIds = blankIds.length > 0 ? blankIds : Object.keys(parseDragAnswer(question.answer));

    if (expectedBlankIds.length === 0) return hasSubmittedAnswer(question, selectedAnswer);
    return expectedBlankIds.every((blankId) => typeof selectedAnswer[blankId] === 'string' && selectedAnswer[blankId].trim() !== '');
  }
  ...
  return hasSubmittedAnswer(question, selectedAnswer);
}
...
export function getQuestionResultState(question, selectedAnswer) {
  ...
  if (question.type === 'MA') {
    const correctAnswer = normalizeMAAnswer(question.answer);
    const selected = normalizeMAAnswer(selectedAnswer);
    return correctAnswer.length > 0
      && selected.length === correctAnswer.length
      && correctAnswer.every((letter, index) => selected[index] === letter)
      ? 'correct'
      : 'wrong';
  }
