...
function buildTfStatementsFromSubs(tfSubs) {
  if (!tfSubs || !Array.isArray(tfSubs) || tfSubs.length === 0) return null;
  return tfSubs.map(sub => sub?.content || '');
}
...
function mapExamFromDb(dbExam) {
  ...
  return {
    ...
    totalQ: questions.length > 0 ? realQuestionCount : (dbExam.total_q || 0),
    createdAt: dbExam.created_at,
    antiCheatEnabled: dbExam.anti_cheat_enabled !== false,
    allowReview: dbExam.allow_review !== false,
    showQuestionLevel: dbExam.show_question_level !== false,
    note: dbExam.note || '',
    mcq,
    ma,
    tf,
    ...
    questions: questions.map(q => {
      ...
      const answer = (q.type === 'TF' && tfSubs)
        ...
        : q.answer;
      const statements = (q.type === 'TF' && tfSubs)
        ? buildTfStatementsFromSubs(tfSubs)
        : stmts;
      return {
        id: q.id,
        type: q.type,
        level: normalizeQuestionLevel(q.level),
        content: q.content,
        options: q.options || [],
        answer,
        solution: q.solution,
...
export async function getPublishedExams() {
  const { data, error } = await supabase
    .from('exams')
    .select('id, title, subject, exam_type, year, duration, published, order_index, folder_id, scoring_config, total_q, created_at, anti_cheat_enabled, allow_review, show_question_level, note, questions(type)')
    .eq('published', true)
    .order('order_index', { ascending: true })
    .order('created_at', { ascending: false });
  if (error) {
...
export async function saveExam(examData) {
  ...
  // 1. Save Exam
  const hasOrderIndex = examData.orderIndex !== undefined && examData.orderIndex !== null && examData.orderIndex !== '';
  const dbExam = {
    ...
    total_q: (examData.questions || []).filter(q => q.type !== 'TEXT').length,
    anti_cheat_enabled: examData.antiCheatEnabled !== false,
    allow_review: examData.allowReview !== false,
    show_question_level: examData.showQuestionLevel !== false,
    note: examData.note || null,
  };

  let savedExamId = examData.id;
  if (hasOrderIndex || !savedExamId) {
    dbExam.order_index = hasOrderIndex ? Number(examData.orderIndex) : 0;
  }
  ...
  if (examData.questions && examData.questions.length > 0) {
    ...
    examData.questions.forEach(q => {
      // Re-use existing UUID if it's not a temporary ID (like NEW_1234)
      const isExistingUuid = q.id && q.id.length === 36 && q.id.includes('-');
      const newUuid = isExistingUuid ? q.id : crypto.randomUUID();
      q._newUuid = newUuid;
      if (isExistingUuid) {
        validUuids.push(newUuid);
      }
      if (q.id) {
        idMap[q.id] = newUuid;
      }
    ...
    // Upsert questions
    const dbQuestions = examData.questions.map((q, index) => {
      const tfStatements = q.type === 'TF'
        ? buildTfStatementsFromSubs(q.tfSubQuestions)
        : null;
      ...
      return {
        ...
        statements: tfStatements || q.statements || null,
        image_url: q.image || null,
        parent_id: q.linkedTo ? (idMap[q.linkedTo] || null) : null
      };
    });
    
    const { error: qError } = await supabase.from('questions').upsert(dbQuestions);
...
export async function getAllFolders() {
  const { data, error } = await supabase
    .from('folders')
    .select('id, name, subject, exam_type, year, visibility, order_index, created_at')
    .order('order_index', { ascending: true })
    .order('created_at', { ascending: false });
...
export async function createFolder(folderData) {
  const dbFolder = {
    ...
    subject: folderData.subject || null,
    exam_type: folderData.examType || null,
    year: folderData.year ? Number(folderData.year) : null,
