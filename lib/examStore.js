...
export async function updateExamsOrder(updates) {
  ...
  // Supabase REST API doesn't support bulk update natively using .update() for different values.
  // Instead, we update one by one. In a real production app we'd use a postgres function (RPC).
  // For simplicity here, we do concurrent updates.
  const promises = updates.map(update => 
    supabase.from('exams').update({ order_index: update.order_index }).eq('id', update.id)
  );
