...
function buildPracticeItem(exam, progress) {
  ...
  return {
    ...
    progressText: completed
      ? (totalQuestions > 0
        ? `Đã hoàn thành ôn luyện · ${revealedCount}/${totalQuestions} câu đã xem · ${answeredCount} câu đã trả lời`
        : 'Đã hoàn thành ôn luyện')
      : (totalQuestions > 0
...
export async function getContinueExamItems(userId, exams = [], folders = []) {
  ...
  try {
    const { data, error } = await supabase
      ...
      .select('exam_id, answered_count, revealed_count, total_questions, completed, saved_at, updated_at')
      .eq('user_id', userId)
