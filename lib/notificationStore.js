...
export async function getPublishedNotifications(userId, { limit = 30 } = {}) {
  ...
  const { data: notifications, error } = await supabase
    .from('notifications')
    .select('id, title, body, image_url, video_url, video_poster_url, media_alt, is_published, published_at, mentioned_exam_ids, created_at, updated_at')
...
export async function getAdminNotifications() {
  const { data, error } = await supabase
    .from('notifications')
    .select('id, title, body, image_url, video_url, video_poster_url, media_alt, is_published, published_at, mentioned_exam_ids, created_by, created_at, updated_at')
    .order('created_at', { ascending: false });
...
export async function saveNotification(notification, currentUserId) {
  ...
  const payload = {
    ...
    mentioned_exam_ids: normalizeUuidList(notification?.mentionedExamIds),
  };
