...
export async function getTargetExams({ includeInactive = false } = {}) {
  let query = supabase
    .from('target_exams')
    .select('id, name, exam_date, is_active, order_index, created_at, updated_at')
...
export async function getUserTargetExams(userId) {
  ...
  const { data, error } = await supabase
    ...
    .select('target_exam_id, target_exams(id, name, exam_date, is_active, order_index, created_at, updated_at)')
    .eq('user_id', userId)
...
export async function saveTargetExam(examData) {
  ...
  if (Number.isFinite(Number(examData.orderIndex))) {
    payload.order_index = Number(examData.orderIndex);
  }
...
export async function updateTargetExamsOrder(orderedExams) {
  for (const [index, exam] of orderedExams.entries()) {
    const { error } = await supabase
      .from('target_exams')
