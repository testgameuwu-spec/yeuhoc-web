import { DEFAULT_QUESTION_LEVEL, normalizeQuestionLevel } from '@/lib/questionLevels';
...
function parseBlock(block) {
    ...
    const options = (type === 'MCQ' || type === 'MA' || type === 'TF' || type === 'DRAG') ? parseOptions(optionsRaw || '', type) : [];

    let finalType = 'SA';
    if (type === 'MCQ') finalType = 'MCQ';
    else if (type === 'MA') finalType = 'MA';
    ...
    if (finalType === 'MA') {
        finalAnswer = normalizeMAAnswerText(finalAnswer);
    }
    ...
    return {
        id: id.trim(),
        type: finalType,
        level: normalizeQuestionLevel(level || DEFAULT_QUESTION_LEVEL),
...
function parseTfAnswer(answerStr) {
    ...
    const regex = /([a-z])\s*[-–:]\s*(Đ|S|đúng|sai|đ|s)/gi;
    let match;
...
function normalizeMAAnswerText(answerStr) {
    if (!answerStr) return '';
    if (/^none$/i.test(String(answerStr).trim())) return '';
...
function parseOptions(optionsText, type = 'MCQ') {
    ...
    if (type === 'TF') {
        // TF options typically use a), b), c), d) or more TSA statements.
