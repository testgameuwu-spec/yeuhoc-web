...
import { Heart, Copy, CheckCircle2, Coffee, Sparkles, ChevronRight, QrCode } from 'lucide-react';
...
export default function DonateWidget({ user }) {
  ...
  // Thông tin ngân hàng mẫu (Người dùng cần thay đổi phần này)
  const BANK_INFO = {
    bankId: 'mbbank', // Mã ngân hàng cho VietQR (vd: vcb, mbbank, techcombank)
    bankName: 'MB Bank',
    accountName: 'YEUHOC',
  ...
  const qrUrl = `https://img.vietqr.io/image/${BANK_INFO.bankId}-${BANK_INFO.accountNumber}-compact2.png?amount=20000&addInfo=${encodeURIComponent(memo)}&accountName=${encodeURIComponent(BANK_INFO.accountName)}`;
      ...
      <div className="relative z-10">
        ...
        {showQR ? (
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 mb-4 animate-fadeIn flex flex-col items-center text-center">
            <img src={qrUrl} alt="QR Code" className="w-48 h-48 object-contain mb-3 rounded-lg" />
