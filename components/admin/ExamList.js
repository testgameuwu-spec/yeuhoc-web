...
import {
  ...
  FileText, Clock, Calendar, GraduationCap, Filter
} from 'lucide-react';
...
export default function ExamList({ exams, onEdit, onDelete, onTogglePublish, onCreateNew }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.03]">
        {/* Header */}
        <div className="grid grid-cols-[1fr_120px_100px_80px_100px_56px] gap-4 px-6 py-3 border-b border-white/10 text-xs font-semibold text-white/30 uppercase tracking-wider">
          <span>Đề thi</span>
        ...
        {filtered.length > 0 ? filtered.map(exam => {
          ...
          return (
            <div key={exam.id}
              className="grid grid-cols-[1fr_120px_100px_80px_100px_56px] gap-4 px-6 py-4 border-b border-white/5 hover:bg-white/[0.03] transition-colors items-center group">
              {/* Title */}
              <div>
              ...
              {/* Status */}
              <div>
                <button onClick={() => onTogglePublish(exam.id)}
